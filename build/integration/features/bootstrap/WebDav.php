...
trait WebDav {
	...
	public function search() {
		$this->response = $this->searchFile($this->currentUser);
	}
	...
	public function searchFile(string $user, ?string $properties = null, ?string $scope = null, ?string $condition = null) {
		...
		if ($properties === null) {
			$condition = '<oc:fileid /> <d:getlastmodified /> <d:getetag />	<d:getcontenttype /> <d:getcontentlength /> <nc:has-preview /> <oc:favorite /> <d:resourcetype />';
		}
	...
	<d:eq>
		...
		if ($scope === null) {
			$scope = '<d:href>/files/admin</d:href><d:depth>infinity</d:depth>';
		}
...
<d:searchrequest xmlns:d="DAV:"
	...
	<d:basicsearch>
		...
		try {
			$this->response = $this->makeDavRequest($user, "SEARCH", '', [], $body, '');
